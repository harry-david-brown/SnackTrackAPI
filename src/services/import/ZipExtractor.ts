 ...
 * 
 * Extracts Uber Eats CSV from ZIP archives
 * Handles Uber's data export structure: [path]/Uber Data/Eats/user_orders-0.csv
 */
...
export class ZipExtractor {
  ...
  validateZipFile(buffer: Buffer, maxSizeMB: number = 50): void {
    ...
    if (!this.isZipFile(buffer)) {
      throw new ValidationError(
        'File is not a valid ZIP archive. Please upload a ZIP file from Uber.',
        'file'
      );
    }
