...
/**
 * @swagger
 * /database/drop-partial-indexes:
 *   post:
 *     summary: Drop partial indexes (one-time migration)
 *     description: Drops the partial indexes that are no longer needed. This is a one-time operation.
 *     tags: [Database]
 *     responses:
 *       200:
 *         description: Partial indexes dropped successfully
 *       500:
 *         description: Internal server error
 */
// POST /database/drop-partial-indexes - Drop partial indexes (one-time migration)
router.post('/drop-partial-indexes', async (req: Request, res: Response) => {
  try {
    const indexesToDrop = [
    ...
    for (const indexName of indexesToDrop) {
      try {
        ...
        const checkResult = await container.postgres.query(`
          SELECT EXISTS (
            SELECT 1 
            FROM pg_indexes 
            WHERE schemaname = 'public' 
        ...
        if (checkResult.rows[0].exists) {
          await container.postgres.query(`DROP INDEX IF EXISTS ${indexName}`);
          results.push({ index: indexName, status: 'dropped' });
        } else {
          results.push({ index: indexName, status: 'not_found' });
        }
      } catch (error: any) {
    ...
    const verifyResult = await container.postgres.query(`
      SELECT indexname 
      FROM pg_indexes
      WHERE schemaname = 'public' 
        AND tablename = 'receipts'
    ...
    res.json({
      message: 'Partial index drop operation completed',
      results,
      verification: {
        remainingPartialIndexes: verifyResult.rows.length,
        indexes: verifyResult.rows.map((r: any) => r.indexname)
      },
      note: 'You can remove this endpoint after confirming indexes are dropped'
    });
  } catch (err) {
    console.error('Error dropping partial indexes:', err);
    res.status(500).json({
      error: 'Failed to drop partial indexes',
      details: err instanceof Error ? err.message : 'Unknown error'
    });
  }
});
